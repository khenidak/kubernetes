...
// Convert_v1_ServiceSpec_To_core_ServiceSpec converts service spec from v1 to core
func Convert_v1_ServiceSpec_To_core_ServiceSpec(in *v1.ServiceSpec, out *core.ServiceSpec, s conversion.Scope) error {
	// convert the automatic part
	if err := autoConvert_v1_ServiceSpec_To_core_ServiceSpec(in, out, s); err != nil {
		return err
	}

	// cluster ip plural in core, single and plural in v1.
	// If both fields (v1.ClusterIPs and v1.ClusterIP) are provided and differ, then ClusterIP is authoritative for compatibility with older kubelets
	if (len(in.ClusterIP) > 0 && len(in.ClusterIPs) > 0) && (in.ClusterIP != in.ClusterIPs[0]) {
		out.ClusterIPs = []string{in.ClusterIP}
	}
	// at the this point, autoConvert copied v1.ClusterIPs -> core.ClusterIPs
	// if v1.ClusterIPs was empty but v1.ClusterIP is not, then set core.ClusterIPs[0] with v1.ClusterIP
	if len(in.ClusterIP) > 0 && len(in.ClusterIPs) == 0 {
		out.ClusterIPs = []string{in.ClusterIP}
	}

	// when user converts to ExternalName they set .spec.clusterIP = ""
	// but current scripts will not set .spec.clusterIPs to nil
	// the below fixes this
	if out.Type == core.ServiceTypeExternalName {
		if len(in.ClusterIP) == 0 && len(out.ClusterIPs) > 0 {
			// reset clusterIPs
			out.ClusterIPs = nil
		}
	}

	// the only case for ClusterIP:"" && ClusterIPs:nil
	// is .spec.Type == ExternalName

	// we need to be careful, if the user send any other garbage ClusterIPs
	// keep it so we can tell'em what is wrong with their request
	if out.Type == core.ServiceTypeExternalName &&
		len(out.ClusterIPs) == 1 &&
		out.ClusterIPs[0] == "" {
		out.ClusterIPs = nil
	}
...
// Convert_core_ServiceSpec_To_v1_ServiceSpec converts service spec from core to v1
func Convert_core_ServiceSpec_To_v1_ServiceSpec(in *core.ServiceSpec, out *v1.ServiceSpec, s conversion.Scope) error {
	if err := autoConvert_core_ServiceSpec_To_v1_ServiceSpec(in, out, s); err != nil {
		return err
	}
	// at the this point autoConvert copied core.ClusterIPs -> v1.ClusterIPs
	// v1.ClusteriP (singular value field, which does not exist in core) needs to
	// be set with core.ClusterIPs[0]
	if len(in.ClusterIPs) > 0 {
		out.ClusterIP = in.ClusterIPs[0]
	}
