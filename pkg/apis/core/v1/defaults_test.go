...
func TestSetDefaultIPFamilies(t *testing.T) {
	preferDualStack := v1.PreferDualStack
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			if len(test.expectedIPFamilies) != len(svc2.Spec.IPFamilies) {
				t.Fatalf("expected .spec.IPFamilies len:%v got %v", len(test.expectedIPFamilies), len(svc2.Spec.IPFamilies))
			}

			for i, family := range test.expectedIPFamilies {
