...
func getAddressTypesForService(service *corev1.Service) map[discovery.AddressType]struct{} {
	...
	// this ensures that this does not happen
	// TODO (khenidak) remove when (1) dual stack becomes
	// enabled by default (2) v1.19 falls of supported versions
	if len(serviceSupportedAddresses) == 0 {
		...
		klog.V(2).Infof("couldn't find ipfamilies for service: %v/%v likely because controller manager is connected to an old apiserver that does not support ip families yet. The service endpoint slice has been detected based on familyOf(ClusterIP:%v). No other ClusterIPs were used", service.Namespace, service.Name, service.Spec.ClusterIP)
	}
