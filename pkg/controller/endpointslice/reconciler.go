...
// reconcileByAddress takes a set of pods currently matching a service selector and
...
func (r *reconciler) reconcile(service *corev1.Service, pods []*corev1.Pod, existingSlices []*discovery.EndpointSlice, triggerTime time.Time) error {
	...
	serviceSupportedAddresses := getAddressTypesForService(service) // addresses that this service supports [o(1) find]

	// loop through slices identify their address type
	...
	for _, existingSlice := range existingSlices {
		...
		if _, ok := slicesByAddressType[existingSlice.AddressType]; !ok {
			slicesByAddressType[existingSlice.AddressType] = make([]*discovery.EndpointSlice, 0)
		}
