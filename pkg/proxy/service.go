...
func (sct *ServiceChangeTracker) newBaseServiceInfo(port *v1.ServicePort, service *v1.Service) *BaseServiceInfo {
	...
	if len(incorrectIPs) > 0 {
		klog.V(4).Infof("service change tracker(%v) ignored the following external IPs(%s) for service %v/%v as they dont match ipfamily", sct.ipFamily, strings.Join(incorrectIPs, ","), service.Namespace, service.Name)
	}

	info.loadBalancerSourceRanges, incorrectIPs = utilproxy.FilterIncorrectCIDRVersion(service.Spec.LoadBalancerSourceRanges, sct.ipFamily)
	if len(incorrectIPs) > 0 {
		klog.V(4).Infof("service change tracker(%v) ignored the following load balancer source ranges(%s) for service %v/%v as they dont match ipfamily", sct.ipFamily, strings.Join(incorrectIPs, ","), service.Namespace, service.Name)
	}
	...
	if len(ips) > 0 {
		...
		if len(incorrectIPs) > 0 {
			klog.V(4).Infof("service change tracker(%v) ignored the following load balancer(%s) ingress ips for service %v/%v as they dont match ipfamily", sct.ipFamily, strings.Join(incorrectIPs, ","), service.Namespace, service.Name)
		}
