...
var map_ServiceSpec = map[string]string{
	...
	"ipFamilies":               "IPFamilies identifies all the IPFamilies assigned for this Service. If a value was not provided for IPFamilies it will be defaulted based on the cluster configuration and the value of service.spec.ipFamilyPolicy. A maximum of two values (dual-stack IPFamilies) are allowed in IPFamilies. IPFamilies field is conditionally immutable it allows for adding another IPFamily or removing the secondary IPFamily. But it does not allow changing the primary IPFamily of the service.",
	...
	"ipFamilyPolicy":           "IPFamilyPolicy represents the dual-stack-ness requested or required by this Service. If there is no value provided, then this Service will be considered SingleStack (single IPFamily). Services can be SingleStack (single IPFamily) PreferedDualStack (two dual-stack IPFamilies on dual-stack clusters or single IPFamily on single-stack clusters). Services can also be RequireDualStack where it will have two dual-stack IPFamilies on dual-stack configured clusters otherwise (on a single stack cluster) it will fail. The IPFamilies and ClusterIPs assigned to this service can be controlled by service.spec.ipFamilies and service.spec.clusterIPs respectively.",
}
